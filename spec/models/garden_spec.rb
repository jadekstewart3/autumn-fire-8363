...
RSpec.describe Garden do
  describe 'relationships' do
    ...
    before (:each) do
      ...
      @peppers_1 = Plant.create!(name: "Jalapenos", description: "Also Green", days_to_harvest: 50)
      ...
      PlantPlot.create!(plot_id: @plot_2.id, plant_id: @peppers.id)
      PlantPlot.create!(plot_id: @plot_2.id, plant_id: @peppers_1.id)
    end
    ...
    it 'returns unique plants that take less than 100 days to harvest' do
      ...
      expect(@garden.unique_plants_less_than_100_days).to_not eq([@tomato.name, @basil.name, @peppers.name, @peppers_1.name])
    end
  end
end
